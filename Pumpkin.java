...
public class Pumpkin extends Actor
{
    ...
    private int numChopsReq = 7; // chops it takes to become diced;
    ...
    private boolean knifeOnPumpkin = false;
    
    private boolean chopped = false; 
    GreenfootSound chopSFX = new GreenfootSound("chop.wav");
    
    public Pumpkin(PlateBack plate, CuttingBoard cuttingBoard, CuttingKnife cuttingKnife) {
        this.plate = plate;
        ...
        if (mouse != null) {
            ...
            if (Greenfoot.mouseDragEnded(this) || (knifeAtObject && Greenfoot.mouseDragEnded(cuttingKnife))) {
                ...
                if (getWorld().getObjectsAt(getX(), getY(), PlateBack.class).contains(plate)) {
                    plate.storeIngredient(0, isChopped, getX(), getY());
    ...
    private void checkIfOnCuttingBoard() {
        //checks whether Pumpkin is on the cuttingboard
    ...
    private void cutPumpkin() {
        if (Greenfoot.mouseClicked(this) && onCuttingBoard && !knifeOnPumpkin) {
        ...
        if (knifeOnPumpkin) {
            ...
            if(framesElapsed >= 10) {
                ...
                knifeOnPumpkin = false;
            }
        }
        if(numChopped == numChopsReq && !isChopped) {
            setImage("pumpkin1.png");
            image.scale(resizedImageWidth,resizedImageHeight);
            image = getImage();
            isChopped = true;
        }
