...
public class Watermelon extends Actor
{
    ...
    private int numChopsReq = 5; // chops it takes to become diced;
    ...
    private boolean knifeOnWatermelon = false;
    
    private boolean chopped = false; 
    GreenfootSound chopSFX = new GreenfootSound("chop.wav");
    
    public Watermelon(PlateBack plate, CuttingBoard cuttingBoard, CuttingKnife cuttingKnife) {
        this.plate = plate;
        ...
        if (mouse != null) {
            ...
            if (Greenfoot.mouseDragEnded(this) || (knifeAtObject && Greenfoot.mouseDragEnded(cuttingKnife))) {
                ...
                if (getWorld().getObjectsAt(getX(), getY(), PlateBack.class).contains(plate)) {
                    plate.storeIngredient(1, isChopped, getX(), getY());
    ...
    private void checkIfOnCuttingBoard() {
        //checks whether Watermelon is on the cuttingboard
